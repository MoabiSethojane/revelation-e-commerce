                    ...
                    return (
                        <div style={{background: 'whitesmoke', margin: 8, color:'grey', display: 'flex'}}>
                           <div style={{padding:'2%'}}><img src={res.image} height={280} alt='product-image' /></div>
                            ...
                            <p>{res.size}</p>
                            <p>{res.productCode}</p>
                            <button className='delete' onClick={(v) =>handleDecrease(res)}><FaMinusCircle size={20} /></button>Quantity:{res.qty}
                            <button className='delete' onClick={(v) =>handleIncrement(res)}>
