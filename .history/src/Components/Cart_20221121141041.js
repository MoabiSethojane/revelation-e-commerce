                    ...
                    return (
                        <div style={{background: 'whitesmoke', margin: 8, color:'grey', display: 'flex'}}>
                           <div style={{padding:'2%'}}><img src={res.image} height={280} alt='product-image' /></div>
                            ...
                            <p>{res.size}</p>
                            <p>{res.productCode}</p>
                            <button onClick={(v) =>handleDecrease(res)}style={{background:'red'}}><FaMinusCircle size={20} /></button>Quantity:{res.qty}<button onClick={(v) =>handleIncrement(res)}style={{background:'green'}}>
                            <FaPlusCircle size={20}/></button>
                            <button className='delete' onClick={(v)=>handleDelete(res)} ><RiDeleteBin5Line size={20}/></button></p>
                            </div>
                        </div>
                    )
                })}
